package com.nlu.app.common.share;

public class SagaAdvancedStep {
    public static final String ENDING_SAGA = "ENDING_SAGA";
    public static final String IDENTITY_CREATE = "IDENTITY_CREATE";
    public static final String PROFILE_CREATE = "PROFILE_CREATE";
    public static final String NOTIFICATION_CREATE = "NOTIFICATION_CREATE";
    public static final String IDENTITY_UPDATE = "IDENTITY_UPDATE";
    public static final String VIDEO_CREATE = "VIDEO_CREATE";
    public static final String VIDEO_UPVOTE = "VIDEO_UPVOTE";
    public static final String VIDEO_DOWNVOTE = "VIDEO_DOWNVOTE";
    public static final String FIRST_INTERACT_WITH_VIDEO = "FIRST_INTERACT_WITH_VIDEO";
    public static final String UPDATE_COMMENT_INTERACTION = "UPDATE_COMMENT_INTERACTION";
}
